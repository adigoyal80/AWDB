...
function vowelCount(str){
    let vowel = 'aeiou';
    return str.split('').reduce(function(acc, nv){
        nv = nv.toLowerCase();
        if(vowel.includes(nv)){
            if (nv in acc){
                acc[nv]++;  
            }
            else {
...
/*
Write a function called addKeyAndValue which accepts an array of objects and returns the array of objects passed 
to it with each object now including the key and value passed to the function.
...
of the callback function at that specific value is false, the value should be placed in the second subarray. 
...
function partition(arr, callback){
    return arr.reduce(function(pv, cv, array){
        if(callback(cv)){
            pv[0].push(cv)
        } else {
            pv[1].push(cv)
